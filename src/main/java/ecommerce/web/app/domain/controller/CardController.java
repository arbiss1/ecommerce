...
public class CardController {
    ...
    public CardController(CardService cardService, PostService postService, UserService userService,
    ...
    public ResponseEntity addToCard(@PathVariable(name = "postId") String postId) throws PostCustomException {
        ...
        Card findIfPostAlreadyExists = cardService.findCardByUserAndPost(getAuthenticatedUser,getFoundPost);
        if(!(findIfPostAlreadyExists == null)){
            return new ResponseEntity("Post is already added to card",HttpStatus.CONFLICT);
        }else {
            return new ResponseEntity(cardService.addToCard(getFoundPost, getAuthenticatedUser), HttpStatus.ACCEPTED);
        }
    }

    @DeleteMapping("/remove-from-card/{postId}")
    public ResponseEntity removeFromCard(@PathVariable(name = "postId") String postId) throws PostCustomException, CardCustomException {
        Optional<Post> findPost = postService.findByPostId(postId);
        User getAuthenticatedUser = userService.getAuthenticatedUser().get();
        if (!findPost.isPresent() || getAuthenticatedUser.equals("")) {
            ...
            Post getFoundPost = findPost.get();
            Card findCardByPostAndUser = cardService.findCardByUserAndPost(getAuthenticatedUser, getFoundPost);
